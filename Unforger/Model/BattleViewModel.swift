...
extension BattleView {
    class ViewModel: ObservableObject {
        ...
        func togglePlayerIdleAnimation(_ enabled: Bool = true) -> Void {
            if enabled {
                self.timerPlayer = Timer.scheduledTimer(withTimeInterval: 0.1, repeats: true) { timer in
        ...
        func toggleOpponentIdleAnimation(_ enabled: Bool = true) -> Void {
            if enabled {
                self.timerOpponent = Timer.scheduledTimer(withTimeInterval: 0.1, repeats: true) { timer in
